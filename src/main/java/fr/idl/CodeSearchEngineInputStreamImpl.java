...
public class CodeSearchEngineInputStreamImpl implements CodeSearchEngineInputStream {
  ...
  static final Logger log = Logger.getLogger(CodeSearchEngineInputStreamImpl.class);
      ...
      while (xmlsr.hasNext() && (exit == false)) {
        ...
        switch (eventType) {
          case XMLEvent.START_ELEMENT:
            ...
            if (in_good_class && in_class_decl_stmt && xmlsr.getLocalName().equals("name")
                && !in_class_decl_stmt_init) {
              in_class_decl_stmt_name = true;
            }

            if (in_good_class && in_class_decl_stmt && xmlsr.getLocalName().equals("index")
                && !in_class_decl_stmt_init) {
              in_class_decl_stmt_name = true;
            }
      ...
      while (xmlsr.hasNext()) {
        ...
        if (classFound) {
          ...
          while (xmlsr.hasNext()) {
            ...
            // class ending
            if (eventType == XMLEvent.END_ELEMENT && xmlsr.getLocalName().equals("class")) {
              break;
            }
            ...
            // DEBUG
            log.debug("Method match - Line : " + xmlsr.getLocation().getLineNumber());
            ...
            while (xmlsr.hasNext()) {
              ...
              switch (eventType) {
                ...
                case XMLEvent.CHARACTERS:
                  builderDOMStructure += Util.escapeSpecialsCaract(xmlsr.getText());
                  break;
            ...
            try {
              ...
              // Type
              Element methodTypeName = rootNode.getChild("type").getChild("name");
              method.setType(new TypeImpl(Util.getFullName(methodTypeName), "", null, null));
              ...
              for (Iterator paramIterator =
                  rootNode.getChild("parameter_list").getChildren("param").iterator(); paramIterator
                  .hasNext();) {
                Element node = (Element) paramIterator.next();

                Element typeNameNode = node.getChild("decl").getChild("type").getChild("name");
                parameters.add(new TypeImpl(Util.getFullName(typeNameNode), "", null,
                    new LocationImpl("")));
              }
          ...
          case XMLEvent.END_ELEMENT:
            ...
            if (xmlsr.getLocalName().equals("function")) {
              ...
              if (typeValue.equals(typeName)) {
                ...
                // Create Method
                MethodImpl method =
                    new MethodImpl(nameValue, new TypeImpl(typeValue, packageValue, kindValue,
                        locationValue), new TypeImpl(), new ArrayList<Type>());
                listMethod.add(method);
              }
  ...
  public List<Method> findOverridingMethodsOf(Method method, InputStream data) {
    ...
    try {
      ...
      while (xmlsr.hasNext()) {
        ...
        if (eventType == XMLEvent.CHARACTERS && xmlsr.getText().equals("@")) {
          eventType = xmlsr.next();

          if (eventType == XMLEvent.CHARACTERS && xmlsr.getText().equals("^function[_A-Za-z]*")) {
  ...
  @Override
  public List<CodeSearchEngine.Method> findMethodsThrowing(String exceptionName, InputStream data) {
    // TODO Auto-generated method stub
    return null;
  }
  ...
  @Override
  public List<Type> findClassesAnnotatedWith(String annotationName, InputStream data) {
    // TODO Auto-generated method stub
    return null;
  }
